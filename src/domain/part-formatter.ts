...
export class PartFormatter {
  constructor(private readonly patchParser = new PatchParser()) {}

  format(part: Part, prefix?: string): string {
    ...
    if (part.type === 'tool') {
      if (part.tool === 'todowrite') {
        const formatted = this.formatTodoList(part)
      ...
      if (part.state.status === 'pending') {
        ...
        const toolTitle = this.formatBashToolTitle({ command, description, summary })
        return `┣ ${pfx}bash${toolTitle}`
      }

      const summaryText = this.getToolSummaryText(part)
      ...
      if (part.state.status === 'error') {
        ...
        const description = (part.state.input?.description as string) || ''
        const summary = (part.state.input?.summary as string) || ''
        const formatted = this.formatBashToolTitle({
          command,
          description,
          summary,
          stateTitle,
  ...
  formatTaskToolTitle(part: Extract<Part, { type: 'tool' }>): string {
  ...
  formatTodoList(part: Part): string {
  ...
  getToolSummaryText(part: Part): string {
    ...
    if (part.tool === 'apply_patch') {
      ...
      if (!patchText) return ''
      return [...this.patchParser.parseFileCounts(patchText).entries()]
